...
const Carousel = () => {
  ...
  const next = () => setCurrentIndex((prev) => (prev + 1) % SHOWCASE_VARIATIONS.length);
  const prev = () => setCurrentIndex((prev) => (prev - 1 + SHOWCASE_VARIATIONS.length) % SHOWCASE_VARIATIONS.length);
  ...
  return (
    <div className="relative max-w-[90vw] mx-auto px-24">
      ...
      <div className="mb-12 max-w-5xl mx-auto">
        <CommandBlock key={v.command} command={v.command} color={v.options.color} />
      </div>

      {/* Larger Terminal with Mac Dots */}
      <div className="flex flex-col bg-black border border-border rounded-[2.5rem] overflow-hidden relative shadow-[0_0_150px_rgba(0,0,0,0.9)] border-opacity-30">
        <div className="bg-[#1a1a1a] p-8 flex items-center gap-4 border-b border-border/30">
          <div className="w-5 h-5 rounded-full bg-[#ff5f56]" />
          <div className="w-5 h-5 rounded-full bg-[#ffbd2e]" />
          <div className="w-5 h-5 rounded-full bg-[#27c93f]" />
          <div className="flex-1 text-center font-mono text-base text-muted tracking-[0.25em] font-bold">
            nihal@rmatrix: ~/{v.title.toLowerCase().replace(" ", "-")}.sh
          </div>
        </div>
